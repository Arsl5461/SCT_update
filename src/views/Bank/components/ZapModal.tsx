...
const ZapModal: React.FC<ZapProps> = ({ onConfirm, onDismiss, tokenName = '', decimals = 18 }) => {
  ...
  return (
    ...
    <div className="modal text-center modal_account">
      <h2>
        {`Zap into ${tokenName}`}
      </h2>
      <StyledActionSpacer />
      <div className="modal_zap_select">
        <span>
          Select asset to zap with
        </span>
        <Select
        ...
        >
          <StyledMenuItem value={FTM_TICKER}>AVAX</StyledMenuItem>
          <StyledMenuItem value={TSHARE_TICKER}>PSHARE</StyledMenuItem>
          {/* Tomb as an input for zapping will be disabled due to issues occuring with the Gatekeeper system */}
          <StyledMenuItem value={TOMB_TICKER}>SCT</StyledMenuItem>
        </Select>
      </div>
      <TokenInput
        onSelectMax={handleSelectMax}
        ...
        symbol={zappingToken}
      />
      <div className="modal-zap-estimate">
        <p>Zap Estimations</p>
        <div className="modal-zap-estimate-input">
          <p>
            {' '}
            {tokenName}: {Number(estimate.token0) / Number(ftmAmountPerLP)}
          </p>
